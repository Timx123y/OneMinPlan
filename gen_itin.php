...
function selectPOI($attractions, $attractions_visited, $itin_node_ids, $itin_edge_ids, $this_day){
	...
	if (count($itin_node_ids[$this_day])>0){
		...
		foreach($sorted_districts as $district){
			...
			if (($new_itin_current_time<=$day_end_time) and ($str !=="")){
...
// Initialization
	...
	*/
	...
	$attractions_seq[0] = array("Peak Tram","Victoria Peak (The Peak)","Peak Tower","Lion's Pavilion at The Peak");
	$attractions_seq[1] = array("Ngong Ping Village (Ngong Ping 360)","Big Buddha","Po Lin (Precious Lotus) Monastery");
	$attractions_seq_1d = array("Peak Tram","Victoria Peak (The Peak)","Peak Tower","Lion's Pavilion at The Peak","Ngong Ping Village (Ngong Ping 360)","Big Buddha","Po Lin (Precious Lotus) Monastery");
	...
	// Should not assume arts and culture have this time limit
	// just set this to be list of museums
	
	/*
	$sql = "SELECT name from attractions_hk_ta WHERE name LIKE \"%museum%\" ";
	$result = mysqli_query($connection, $sql);
	if (mysqli_num_rows($result) > 0){
		while($row = mysqli_fetch_assoc($result)) {
			array_push($attractions_arts , $row["name"]);		// push arts and culture attractions
		}
	}
	*/	
...
while($unvisited_num>0){
	...
	if ($new_POIs === false){
		...
		if (	($new_POIs[0] =="D")or ($new_POIs[0]=="L")){
			...
			if (count($new_POIs)>1) {
				...
				if (count($itin_node_ids[$this_day])>=3){
					...
					$sql = "SELECT * from graph_hk_ta where start_point = \"".  mysqli_real_escape_string($connection,$current_node_name). "\" and end_point = \"".mysqli_real_escape_string($connection,$next_node_name).  "\" and travel_time>0 and travel_mode =\"walk\" order by travel_time";
					$result = mysqli_query($connection, $sql);
					$row = mysqli_fetch_assoc($result);
					
					if (is_null($row)) {
						//  add walk edge						
						$walk_edge_id=addEdgeDB($current_node_name,$next_node_name,"walk");
						updateEdgeDB($walk_edge_id,"walk");
						$sql = "SELECT * from graph_hk_ta where edge_id = $walk_edge_id";
						$result = mysqli_query($connection, $sql);
						$row = mysqli_fetch_assoc($result);
					...
					if ($walk_time>20){
						...
						$row = mysqli_fetch_assoc($result);
						if (is_null($row)){
							$transit_edge_id=addEdgeDB($current_node_name,$next_node_name,"transit");
							updateEdgeDB($transit_edge_id,"transit");
							$sql = "SELECT * from graph_hk_ta where edge_id = $transit_edge_id";
							$result = mysqli_query($connection, $sql);
							$row = mysqli_fetch_assoc($result);
						...
						$transit_time = $row["travel_time"];
						$transit_edge_id = $row["edge_id"];
						array_push($itin_edge_ids[$this_day], $transit_edge_id);								
					} else {
						array_push($itin_edge_ids[$this_day], $walk_edge_id);
					}
		...
		if (count($itin_node_ids[$this_day])>0){
			...
			$sql = "Select * from graph_hk_ta where travel_mode =\"walk\"  and start_point=\"". mysqli_real_escape_string($connection,$last_point)."\" and end_point =\"" . mysqli_real_escape_string($connection,$new_POIs[0]) . "\"  order by travel_time"; 
			$result = mysqli_query($connection, $sql);
			$row = mysqli_fetch_assoc($result);
			
			if (is_null($row)){
				$walk_edge_id=addEdgeDB($last_point,$new_POIs[0],"walk");
				updateEdgeDB($walk_edge_id,"walk");
				$sql = "Select * from graph_hk_ta where edge_id =$walk_edge_id";
				$result = mysqli_query($connection, $sql);
				$row = mysqli_fetch_assoc($result);
			...
			// if walk edge is too long, then search/add transit edge
			if ($walk_time>20) {
				$sql = "Select * from graph_hk_ta where start_point=\"". mysqli_real_escape_string($connection,$last_point)."\" and end_point =\"" . mysqli_real_escape_string($connection,$new_POIs[0]) . "\"  and  travel_time>0 and travel_mode !=\"walk\" order by travel_time"; 
				$result = mysqli_query($connection, $sql);
				$row = mysqli_fetch_assoc($result);
				if (is_null($row)){
					$transit_edge_id=addEdgeDB($last_point,$new_POIs[0],"transit");
					updateEdgeDB($transit_edge_id,"transit");
					$sql = "Select * from graph_hk_ta where edge_id =$transit_edge_id";
					$result = mysqli_query($connection, $sql);
					$row = mysqli_fetch_assoc($result);				
		...
		for($x=1;$x<count($new_POIs);$x +=1){
			...
			$sql = "Select edge_id from graph_hk_ta where start_point=\"".mysqli_real_escape_string($connection,$new_POIs[$x-1])."\" and end_point =\"" . mysqli_real_escape_string($connection,$new_POIs[$x]) . "\" order by travel_time"; 
			$result = mysqli_query($connection, $sql);
			$row = mysqli_fetch_assoc($result);
			
			if($row===false){

				$this_edge_id=addEdgeDB($new_POIs[$x-1],$new_POIs[$x],"transit");
				updateEdgeDB($this_edge_id,"transit");
				
				// if straight line distance <= 2km, then also add walk edge
				if ($distsq <=4000000){
					$this_edge_id=addEdgeDB($new_POIs[$x-1],$new_POIs[$x],"walk");
					updateEdgeDB($this_edge_id,"walk");
				}			
			$sql = "Select edge_id from graph_hk_ta where start_point=\"".mysqli_real_escape_string($connection,$new_POIs[$x-1])."\" and end_point =\"" . mysqli_real_escape_string($connection,$new_POIs[$x]) . "\" order by travel_time"; 
			$result = mysqli_query($connection, $sql);
			...
			array_push($itin_edge_ids[$this_day], $row["edge_id"]);		
			$outputtext = "<script> console.log('itin_edge_ids[$this_day] sql: ". $sql . "');</script>";
			echo 	$outputtext ;
...
// if last attraction on last day is too weird, e.g. takes too much travel time or exceed time limit, then discard it

/*
if (count($itin_node_ids[$days])>=2){
	$last_node = $itin_node_ids[$days][count($itin_node_ids[$days])-1];
	$last_edge = $itin_edge_ids[$days][count($itin_edge_ids[$days])-1];

	while ((getItinTime(array(),array($last_edge))>60) or (60*$starttimeh+getItinTime($itin_node_ids[$days],$itin_edge_ids[$days])>$day_end_time)){
		// remove last point
		$itin_node_ids[$days] = array_slice($itin_node_ids[$days],0,count($itin_node_ids[$days])-1);
		$itin_edge_ids[$days] = array_slice($itin_edge_ids[$days],0,count($itin_edge_ids[$days])-1);
		$last_node = $itin_node_ids[$days][count($itin_node_ids[$days])-1];
		$last_edge = $itin_edge_ids[$days][count($itin_edge_ids[$days])-1];
	}
}
